...
const HeroSection = ({ onExplore }: HeroSectionProps) => (
  <section id="prologue" className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 z-10">
    ...
    <div className="flex items-center gap-4 mt-16 animate-fade-up-delay-3 cursor-pointer" onClick={onExplore}>
      <span className="w-12 h-px bg-gold" />
      <span className="text-gold-accent text-xs uppercase tracking-widest font-body animate-blink hover:text-cream-accent transition-colors">Click here to see the magic</span>
      <span className="w-12 h-px bg-gold" />
    </div>
  </section>
);
